...
class FirmwareCIWorkflowAssemblyOnlyTests(unittest.TestCase):
    ...
    def test_workflow_downloads_matching_components_with_fail_fast(self):
        ...
        self.assertIn("helloworld-component-${{ needs.resolve-version.outputs.version }}", self.workflow_content)
        self.assertIn("fullcone-component-${{ needs.resolve-version.outputs.version }}", self.workflow_content)
        self.assertIn("uses: actions/download-artifact@v6", self.workflow_content)
        self.assertNotIn("gh run download", self.workflow_content)
        self.assertNotIn("gh release download", self.workflow_content)
        self.assertNotIn("GITHUB_RUN_ID", self.workflow_content)
